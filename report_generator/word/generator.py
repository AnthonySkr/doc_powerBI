...
from report_generator.merge import ChangeLog, apply_merge, markers, orphans, read_previous
from report_generator.word import word_app
from report_generator.word.builder import DocumentBuilder, TextProvider
from report_generator.word.errors import DocumentError
...
def _template_path(config: DocConfig, context: dict[str, Any]) -> Path:
    ...
    near = config.path.resolve().parent if config.path else None
    path = paths.find(name, near=near)
    if path.is_file():
        return path

    cherches = "\n".join(f"      {c.absolute()}" for c in paths.candidates(name, near))
    raise DocumentError(
        f"Template introuvable : '{name}'. Cherché ici :\n{cherches}\n"
        f"      Placez-le à côté de {config.path.name if config.path else 'la configuration'}."
    )
