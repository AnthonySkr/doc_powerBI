...
`PowerBiMetadata` est ce qui passe de main en main : l'extraction le produit,
la capture l'enrichit de ses images, le document le consomme.
"""
...
class PowerBiMetadata:
    ...
    `pbi_extractor` le produit en lisant le `.pbip`, `gui_automator` y ajoute
    l'inventaire de ses captures, `report_generator` s'en sert pour écrire le
    document. Un seul objet, passé de main en main : aucun des trois n'a à
    retourner à la source.
    """

    report: PowerBIReport
    # Le projet .pbip dont tout ceci a été tiré.
    source: Path = Path()
