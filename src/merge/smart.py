...
class _Merger:
    ...
    def _without_promoted(self, element_id: str, nodes: list) -> list:
        ...
        Le rapprochement est délibérément étroit : seul le titre d'un élément
        neuf est écarté, et seulement du contenu libre de l'élément qui le
        précède immédiatement. Deux parties homonymes ailleurs dans le document
    ...
    def _segment(self, fresh: Segment | None, old: Segment | None, changed: bool) -> list:
        ...
        if fresh is None:
            # Bloc retiré du plan : ses données ne sont plus écrites, mais ce
            # que l'utilisateur avait glissé dedans lui revient.
            return self._salvaged(old, changed) if old is not None else []
        if old is None:
    ...
    def _salvaged(self, old: Segment, changed: bool) -> list:
        """Ce que l'utilisateur avait écrit dans un bloc que le plan a perdu."""
        return self._preserved([node for _, node in salvage.of(old)], changed)
    ...
    def _mark_review(self, node, changed: bool) -> None:
        ...
        _clear_highlight(node, _signal_colors(self.options))
...
def _weave(old_ids: list[str], fresh_ids: list[str]) -> list[str]:
    ...
    for block_id in fresh_ids:
        if block_id in common:
            previous = block_id
        elif block_id not in following.get(previous, ()):
...
def _untouched(segment: Segment) -> bool:
    """Le contenu du segment est-il exactement celui que le script y avait mis ?"""
    if segment.digests is None:
        # Document d'une version antérieure : sans empreintes, on ne sait pas.
        # Dans le doute, ce qui s'y trouve appartient à l'utilisateur.
        return False
    return [markers.digest(node) for node in segment.content_nodes()] == list(segment.digests)
...
def _signal_colors(options: dict[str, Any]) -> set[str]:
    """Couleurs que le script emploie pour signaler — les seules qu'il retire."""
    return {
        color
        for key in ("highlight_changed", "highlight_new")
        if (color := _HIGHLIGHTS.get(str(options.get(key, "")).lower())) is not None
    }
